...
for iFiles = 1:numel(sDir)
    if ~strcmp(sDir(iFiles).name, '.') && ~strcmp(sDir(iFiles).name, '..') && sDir(iFiles).isdir
        iRuns = iRuns+1;
        % Here 'nirs' is added for BIDS-compatible files
        currDirMat = dir(fullfile(currentDir, sDir(iFiles).name, 'nirs', '*.mat'));
        % Always consider the first file to contain the resting-sate data
        % usually '1_resting.mat'
        % Here 'nirs' is added for BIDS-compatible files
        runName{iRuns} = fullfile(currentDir, sDir(iFiles).name, 'nirs', currDirMat(1).name);
